 ...
 */
#define _THIS_MALLOC		pr_malloc
//#define _THIS_CALLOC		pr_calloc
#define _THIS_REALLOC		pr_realloc
#define _THIS_FREE			pr_free
...
#define _SET_HOOKS			obj_SetHooks
#define _FETCH_HOOKS		obj_FetchHooks
#endif
...
#define SIMPLELOCK_DECL(lock)	DECLARE_PR_LOCK(lock)
#define SIMPLELOCK_LOCK(lock)	pr_lock(&lock)
#define SIMPLELOCK_UNLOCK(lock)	pr_unlock(&lock)
 ...
 * This malloc lock
 */
static SIMPLELOCK_DECL(__malloc_lock);
...
_THIS_MALLOC(size_t nbytes)
{
	...
	if ((nbytes - 1) < SMALL_REQUEST_THRESHOLD) {
		...
		*(block **)bp = arenalist;
		...
		watermark = 0;
		/* Page-round up */
		arenaproto = bp + (SYSTEM_PAGE_SIZE -
