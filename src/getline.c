...
#if defined(__hpux) || defined(osf_)       /* W.Karig@gsi.de */
#ifndef unix
#define unix
#endif
#endif
...
#ifdef unix
#include <unistd.h>
#if !defined(osf_) && !defined(_AIX)       /* W.Karig@gsi.de */
#include <sys/ioctl.h>
#endif
...
#ifdef vms
    descrip.dsc$w_length  = (int) strlen("tt:");
...
Getlinem(int mode, const char *prompt)
{
    ...
    while ((c = gl_getc()) >= 0) {
        ...
        if (c >= (' '+1)) {
			...
			case ' ':
				 if (Gl_spc_hook) {
                      tmp = gl_pos;
                      loc = Gl_spc_hook(gl_buf, (int) strlen(gl_prompt), &tmp);
            ...
            case '\006':                                      /* ^F */
                 gl_fixup(gl_prompt, -1, gl_pos+1);
                 break;
            case '\010': case '\177':						  /* ^H and DEL */
				if (Gl_bsp_hook) {
                      tmp = gl_pos;
                      loc = Gl_bsp_hook(gl_buf, (int) strlen(gl_prompt), &tmp);
                      if (loc >= 0 || tmp != gl_pos || loc == -2)
                           gl_fixup(gl_prompt, loc, tmp);
                 } else 
					gl_del(-1);     
  ...
  */
 {
     ...
     if (change == -2) {   /* reset */
         ...
         if ((int) strlen(prompt) > (BUF_SIZE-1)) {
     ...
     gl_pos = cursor;
     setCursorPosition(gl_pos + (int) strlen(prompt) - gl_shift);
 }
 ...
 Gl_histadd(char *buf)
 {
     ...
     if (*p) {
         ...
         if (strchr(p, '\n'))    /* previously line already has NL stripped */
             len--;
         if (prev == 0 || (int)(int) strlen(prev) != len ||
		   ...
		   apr_snprintf(tname, sizeof(tname), "%s.tmp", gl_histfile);
